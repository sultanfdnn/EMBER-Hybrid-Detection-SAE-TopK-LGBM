...
from catboost.utils import get_gpu_device_count  #  CatBoost GPU kontrolü için
_cat_task = "GPU" if get_gpu_device_count() > 0 else "CPU"  #GPU varsa GPU, yoksa CPU
...
def bulletproof_clean(data):
    ...
    clean_data = np.asarray(data)  # dtype'i KORU (float32 ise 32 kalır)
...
# === (YENİ) Eğitim süreleri için sözlük ===
import time
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    # ✅ fit_transform çıktısının ilk elemanı X_scores olduğu için [0] ekliyoruz
    ...
    datasets = {
        "PLS-DA": (X_train_pls, y_train, X_test_pls),  # DOĞRU: PLS skorları
    }
...
if 'all_results' in locals() and all_results:
    ...
    # ==== (YENİ) Eğitim süresini sonuç tablosuna ekle ====
    # Not: Eğitim süresi "model başına ve fold bazında" tek değerdir; Train/Test satırlarının ikisine de aynı süre yazılır.
