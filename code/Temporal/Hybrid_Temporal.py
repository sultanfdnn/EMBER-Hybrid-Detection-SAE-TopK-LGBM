...
# ========================= Eşik / Hedef =========================
# "youden"  -> Youden J = TPR - FPR
# "multi"   -> çok-ölçütlü hedef (>=TARGET_ALL) + F1xBA yedeği
THRESH_METHOD = "multi"
TARGET_ALL    = 0.97
...
def find_best_threshold_multi(y_true, y_prob, target=0.97):
    # 1) Tüm metrikler hedefi geçen varsa: F1 en yüksek olanı seç
    cands = []
...
x  = layers.Dropout(0.05)(x)  # 0.15 -> 0.05
...
autoenc.fit(
    Xtr32, Xtr32,
    epochs=100,              # 40 -> 100
    batch_size=256,          # 512 -> 256 (daha fazla güncelleme)
    verbose=0
)
